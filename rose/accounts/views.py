...
def signup(request, signup_form=SignupForm,
    ...
    **Context**
    ...
    if request.method == 'POST':
	    ...
	    message = message + u' .Palveluun on rekisteröitynyt uusi käyttäjä. Tarkista tiedot ja lähetä maksu. Käyttäjätunnus on: '+'%s' %(form.cleaned_data['username'])
	    sender = 'tapio.tomi@gmail.com'
	    receiver = ['tapio.tomi@gmail.com']
	    send_mail('Uusi käyttäjä rekisteröitynyt', message,sender,receiver)
...
def profile_pyynnot(request, username, template_name='userena/pyynnot_detail.html', success_url=None,
     ...
     form = TarjousModel.objects.filter(toimiala=profile.toimiala,is_public=True)
    # If no usernames are wanted and the default form is used, fallback to the
    # default form that doesn't display to enter the username.
 ...
 #       'form': form
  #  })
    # return render_to_response('profile_detail.html', {'form': p},
     ...
     return render_to_response('userena/pyynnot_detail.html',
		{'form': form,},
		context_instance=RequestContext(request))
@secure_required
@permission_required_or_403('change_profile', (get_profile_model(), 'user__username', 'username'))
def delete_tp(request, username,id):
   #+some code to check if New belongs to logged in user
   u = TyopaikkaModel.objects.get(pk=id).delete()
   return HttpResponseRedirect('/accounts/anonymous/tyopaikat/deletecomplete/')
   #return  render_to_response('userena/delete_tp.html')
 #  pass
...
@secure_required
@permission_required_or_403('change_profile', (get_profile_model(), 'user__username', 'username'))
def tyopaikat(request, username,success_url=None,extra_context=None, **kwargs):
  if request.method == 'POST': 
    user = get_object_or_404(User,
                             username__iexact=username)
    #form = TyopaikkaModelForm(request.POST)
    profile = user.get_profile()
    
    user_initial = {'owner': username}
 ...
 #   form = TyopaikkaModelForm(request.POST)
	   ...
	   # instance.owner_id =user.id
	    instance.save()
    	    if success_url: redirect_to = success_url
            else: redirect_to = reverse('userena_tyopaikat_complete', kwargs={'username': username})
            return redirect(redirect_to)
  else:
      form = TyopaikkaModelForm() # An unbound form

  return render(request, 'userena/tyopaikka_form.html', {
      'form': form
  })
def tyopaikat_detail(
    request, username,
    template_name=userena_settings.USERENA_TYOPAIKKA_DETAIL_TEMPLATE,
    extra_context=None, **kwargs):
    ...
    if not profile.can_view_profile(request.user):
        return HttpResponseForbidden(_("Sinulla ei ole oikeuksia tähän profiiliin."))

    html = []
    from accounts.models import MyProfile
    geh = 'anonymous'
    gehe = 'anonymous'
    for w in TyopaikkaModel.objects.filter(owner__username__iexact=username):
	if w.haku_viimeistaan > datetime.date.today():
#	  if w.yrityksen_nimi='anonymous':
		idd = w.id
	#	username = w.username
		...
		otsikko = w.otsikko
		kuvaus = w.kuvaus
		html.append(dict(yritys=yritys,email=email,etunimi=etunimi,sukunimi=sukunimi,toimiala=toimiala,alue=alue,haku_viimeistaan=haku_viimeistaan,otsikko=otsikko,kuvaus=kuvaus,idd=idd))
    return render_to_response('userena/tyopaikka_detail.html',
	{'form': html,},
	context_instance=RequestContext(request))
