...
from tarjoukset.models import TarjousModel
from django.db import models
...
class UserenaBaseProfile(models.Model):
    ...
    mugshot = ThumbnailerImageField(_('tunnistuskuva'),
                                    blank=True,
				    editable=False,
                                    upload_to="dummy",#upload_to_mugshot,
                                    resize_source=MUGSHOT_SETTINGS,
                                    help_text=_('Profiilin tunnistuskuva.'))

    privacy = models.CharField(_('yksityiyys'),
                               max_length=15,
		               editable=False,
                               choices=PRIVACY_CHOICES,
                               default=userena_settings.USERENA_DEFAULT_PRIVACY,
                               help_text = _('Kertoo kuka voi katsoa profiiliasi.'))
...
class MyProfile(UserenaBaseProfile):
 ...
 #   toimiala = forms.ModelChoiceField(queryset=PhoneNo.objects.all())
 #   creator = models.ForeignKey(User, related_name='created_projects')
 #   colour = models.IntegerField(choices=[(1, 'Red'), (2, 'Blue'),])

    
 #   etunimi = models.CharField(max_length=80, null=True, verbose_name=('Etunimi'))
  #  sukunimi = models.CharField(max_length=80,null=True, verbose_name=('Sukunimi'))

    yrityksen_nimi= models.CharField(max_length=80,blank=False, verbose_name=('Yrityksen nimi'))
 
    y_tunnus = models.CharField(max_length=80,blank=False, verbose_name=('Y-Tunnus'))
    toimiala = models.ForeignKey(Toimiala, blank=False, null=True, verbose_name=('Toimiala'))
    puhelinnumero= models.CharField(max_length=80,blank=False, verbose_name=('Puh.numero'))
   

class TarjousModelFormPreview(FormPreview):
    def done(self, request, cleaned_data):
	return HttpResponseRedirect('/form/succes')
